const ar = {
  next: 'التالي',
  free: 'مجاني',
  mapLabel: 'الخريطة',
  type: 'نوع',
  experience: 'نوع التجربة',
  person: 'شخص',
  'search-destinations': 'ابحث عن الوجهات',
  'check-in': 'تسجيل الوصول',
  'check-out': 'تسجيل المغادرة',
  'add-date': 'إضافة تاريخ',
  'add-guests': 'إضافة ضيوف',
  who: 'من',
  where: 'الوجهة',
  background: 'الخلفية',
  leaf: 'الورقة',
  guests: {
    adults: 'البالغون',
    adultsDescription: '12 سنة أو أكثر',
    children: 'الأطفال',
    childrenDescription: 'من 3 إلى 11 سنة',
    infants: 'الرضع',
    infantsDescription: 'أقل من 3 سنوات',
    from: 'من',
    perPerson: 'شخص',
  },
  experienceTypes: {
    sunDriedTomato: 'تجربة صنع وتجفيف الطماطم',
    orangePicking: 'تجربة قطف البرتقال والحمضيات',
    treePlanting: 'تجربة زراعة الأشجار',
    wineTasting: 'تجربة تذوق النبيذ وجولة في الكروم',
  },
  home: 'الرئيسية',
  'about-us': 'من نحن',
  news: 'الأخبار',
  gallery: 'معرض الصور',
  'contact-us': 'اتصل بنا',
  experiences: 'التجارب',
  events: 'الفعاليات',
  stays: 'الإقامات',
  'offers-packages': 'العروض والباقات',
  products: 'المنتجات',
  'view-all': 'عرض الكل',
  statistics: {
    agritourismExperiences: 'تجارب السياحة الزراعية',
    yearsOfExperience: 'سنوات الخبرة في السياحة',
    womenEmpowered: 'النساء المستفيدات',
    tourists: 'السياح الذين يثقون بنا',
    yearlyExperiences: 'تجارب تضاف سنويًا',
  },
  aboutUs: {
    title: 'من نحن',
    description:
      'تأسست بوك أجري في عام 2015 بفكرة ربط المزارعين والتجارب الزراعية مع السياح المحليين والأجانب. كل يوم، يقدم المضيفون تجارب سياحية زراعية فريدة وإقامات ريفية تتيح للضيوف التواصل مع المجتمعات الريفية بطريقة أكثر أصالة.',
    vision: {
      title: 'رؤيتنا',
      description:
        'تتطلع بوك أجري إلى أن تكون منصة تفاعلية عالمية رائدة للسياحة الزراعية حيث يمكن لأي شخص أن ينتمي إلى أي تجربة سياحية زراعية في أي وقت وأي مكان باستخدام جهازه.',
    },
    mission: {
      title: 'مهمتنا',
      description:
        'تسعى بوك أجري إلى جلب جميع أعمال السياحة الزراعية إلى منصتها حيث يمكن للجميع التواصل مع مزارعهم الداخلي في أي قرية أو مزرعة أو ريف في جميع أنحاء العالم.',
    },
    readMore: 'اقرأ المزيد',
  },
  downloadApp: {
    subtitle: 'قم بتنزيل تطبيق بوك أجري',
    title: 'اختر تجربتك، وأنشئ ذكريات تدوم مدى الحياة',
    description:
      'يجمع تطبيق بوك أجري بين مجتمع عالمي من الضيوف والمضيفين. يمكنك العثور على مزرعتك المثالية أو الإقامة الريفية مع تجارب السياحة الزراعية الأكثر متعة وتفاعلاً بقيادة السكان المحليين بالإضافة إلى الأطعمة والحرف اليدوية المصنوعة محليًا.',
    googlePlay: 'جوجل بلاي',
    appStore: 'آب ستور',
  },
  contactUs: {
    subtitle: 'اتصل بنا',
    title: 'هل لديك سؤال أو ملاحظة؟',
    email: {
      title: 'البريد الإلكتروني',
      value: 'info@bookagri.com',
    },
    phone: {
      title: 'الهاتف',
      value: '00962-77-2236393',
    },
    address: {
      title: 'العنوان',
      value: 'مرج الحمام، عمّان، الأردن',
      zipCode: 'الرمز البريدي: 11733',
    },
    socialMedia: {
      facebook: 'فيسبوك',
      linkedin: 'لينكد إن',
      instagram: 'إنستجرام',
      tiktok: 'تيك توك',
      whatsapp: 'واتساب',
    },
  },
  contactUsForm: {
    title: 'املأ النموذج',
    subtitle: 'نستمع بعناية لآرائك واقتراحاتك',
    firstName: 'الاسم الأول',
    lastName: 'اسم العائلة',
    email: 'البريد الإلكتروني',
    phone: 'رقم الهاتف',
    message: 'اكتب رسالتك هنا',
    submit: 'إرسال الرسالة',
    successMessage: 'تم إرسال رسالتك بنجاح. سنتواصل معك في أقرب وقت ممكن',
    errorMessage: 'حدث خطأ ما، يرجى المحاولة لاحقًا!',
  },
  footer: {
    pages: 'الصفحات',
    contactUs: 'اتصل بنا',
    email: {
      title: 'البريد الإلكتروني',
      value: 'info@bookagri.com',
    },
    phone: {
      title: 'الهاتف',
      values: [
        '00962-77-2236393',
        '00962-77-0504153',
        '00962-78-7877885',
        '00962-79-5593907',
      ],
    },
    address: {
      title: 'العنوان',
      value: 'مرج الحمام، عمّان، الأردن',
      zipCode: 'الرمز البريدي: 11733',
    },
    hours: '09:00 - 17:00',
    copyright: 'جميع الحقوق محفوظة © Bookagri.com',
  },
  'page-not-found': 'الصفحة غير موجودة',
  'go-back': 'العودة',
  'take-me-home': 'العودة للرئيسية',
  welcome: 'مرحبا',
  to: 'إلى',
  login: 'تسجيل الدخول',
  signup: 'إنشاء حساب',
  email: 'البريد الإلكتروني',
  password: 'كلمة المرور',
  'forgot-password': 'نسيت كلمة المرور؟',
  'dont-have-account': 'ليس لديك حساب؟',
  or: 'أو',
  'error-404': 'خطأ 404',
  'page-not-found-desc':
    'عذرًا، الصفحة التي تبحث عنها غير موجودة. إليك بعض الروابط المفيدة:',
  signin: 'تسجيل الدخول',
  'signing-in': 'جاري تسجيل الدخول...',
  from: 'من',
  exploreNow: 'استكشف الآن',
  wishlist: {
    wishlist: 'المفضلة',
    your: 'قائمة',
    title: 'أنشئ قائمة المفضلة الخاصة بك!',
    description: 'ابدأ بإضافة التجارب والإقامات والفعاليات والمنتجات اليوم.',
    create: 'استكشف الآن',
    countLabel: 'لديك',
    collection: 'مجموعة',
    saved: 'محفوظ',
    delete: 'حذف',
    edit: 'تعديل',
    deleteConfirmation: {
      title: 'حذف المجموعة',
      message: 'هل أنت متأكد أنك تريد حذف هذه المجموعة؟',
      confirmText: 'حذف',
      cancelText: 'إلغاء',
    },
    deleteSuccess: 'تم حذف المجموعة بنجاح',
    editConfirmation: {
      title: 'تعديل المجموعة',
      message: 'هل أنت متأكد أنك تريد تعديل هذه المجموعة؟',
      confirmText: 'تعديل',
      cancelText: 'إلغاء',
    },
    editModal: {
      title: 'تعديل المجموعة',
      collectionNameLabel: 'اسم المجموعة',
      collectionNamePlaceholder: 'أدخل اسم المجموعة',
      validation: {
        nameRequired: 'اسم المجموعة مطلوب',
        nameUnchanged: 'يرجى إدخال اسم مختلف',
      },
    },
  },
  notifications: {
    notifications: 'الإشعارات',
    today: 'اليوم',
    yesterday: 'أمس',
    previousDays: 'الأيام السابقة',
    markAllAsRead: 'تعليم الكل كمقروء',
    deleteAll: 'حذف الكل',
    markAsRead: 'تعليم كمقروء',
    delete: 'حذف',
    noNotifications: 'لا توجد إشعارات',
    markAsUnread: 'تعليم كغير مقروء',
  },
  profile: {
    personalInfo: 'المعلومات الشخصية',
    editPersonalInfo: 'تعديل معلوماتك الشخصية',
    security: 'الأمان',
    changePassword: 'تغيير كلمة المرور',
    myBookings: 'حجوزاتي',
    bookingSettings: 'عرض إعدادات الحجز',
    currency: 'العملة',
    language: 'اللغة',
    support: 'الدعم',
    helpCenter: 'مركز المساعدة',
    contactUs: 'اتصل بنا',
    faqs: 'الأسئلة الشائعة',
    haveQuestions: 'هل لديك أي أسئلة؟',
    aboutBookagri: 'حول بوك أجري',
    missionVision: 'المهمة والرؤية',
    privacyPolicy: 'سياسة الخصوصية',
    checkPrivacy: 'اطلع على سياسة الخصوصية لدينا',
    termsOfService: 'شروط الخدمة',
    checkTerms: 'اطلع على شروط الخدمة لدينا',
    logout: 'تسجيل الخروج',

    firstName: 'الاسم الأول',
    lastName: 'اسم العائلة',
    phoneNumber: 'رقم الهاتف',
    birthdate: 'تاريخ الميلاد',
    gender: 'الجنس',
    male: 'ذكر',
    female: 'أنثى',
    nationality: 'الجنسية',
    country: 'الدولة',
    city: 'المدينة',
    location: 'الموقع',
    save: 'حفظ',
    cancel: 'إلغاء',

    oldPassword: 'كلمة المرور الحالية',
    newPassword: 'كلمة المرور الجديدة',
    confirmPassword: 'تأكيد كلمة المرور',
    changePasswordBtn: 'تغيير كلمة المرور',

    selectCurrency: 'اختر العملة',
    currencyDescription: 'اختر العملة المفضلة لعرض الأسعار',
    currencies: {
      usd: 'دولار أمريكي',
      jod: 'دينار أردني',
      aud: 'دولار أسترالي',
      brl: 'ريال برازيلي',
      gbp: 'جنيه استرليني',
      eur: 'يورو',
    },
    updateUser: {
      personalInfoUpdated: 'تم تحديث المعلومات الشخصية بنجاح',
      personalInfoUpdateFailed: 'فشل تحديث المعلومات الشخصية',
    },
    securityPage: {
      changePasswordSuccess: 'تم تغيير كلمة المرور بنجاح',
      changePasswordError: 'فشل تغيير كلمة المرور',
    },
  },
  myBookings: {
    myBookings: 'حجوزاتي',
    title: 'حجوزاتي',
    description: 'عرض حجوزاتك وإدارتها',
    noBookings:
      'لم تقم بأي حجز حتى الآن! انطلق إلى عالم السياحة الزراعية وابدأ رحلتك.',
    searchPlaceholder: 'ابحث برقم الحجز',
    slogan: 'أنا بوك أجري، وأنت؟',
  },
  bookingStatus: {
    pending: 'في الانتظار',
    approved: 'معتمد',
    declined: 'مرفوض',
    cancelled: 'ملغي',
    refunded: 'مسترد',
    completed: 'مكتمل',
    refundRequested: 'طلب استرداد',

    upcoming: 'قادمة',
    cancel: 'إلغاء',
    cancelBooking: 'إلغاء الحجز',
    bookingStatus: 'حالة الحجز',
    cancellationPolicyModal: {
      title: 'إلغاء الحجز',
      cancelPolicy: 'سياسة الإلغاء',
      sendEmail: 'في حالة الإلغاء أرسل لنا بريدًا إلكترونيًا',
      cancellationCost: 'تكلفة الإلغاء',
    },
    cancellationReasonsModal: {
      reasons: 'أخبرنا سبب إلغاء الحجز',
      send: 'إرسال',
      reasonRequired: 'السبب مطلوب',
      maxLength: 'الحد الأقصى 450 حرف',
      minLength: 'الحد الأدنى 10 حرف',
    },
    cancellationSuccess: {
      title: 'تم إلغاء الحجز',
      description:
        'ستتلقى رصيدًا استرجاعيًا بناءً على سياسة الإلغاء. سنرسل لك بريدًا إلكترونيًا يؤكد إلغاء الحجز. لا تحتاج إلى أن تفعل شيئًا آخر!',
      emailNote:
        'إذا لم تستلم البريد الإلكتروني، يرجى التحقق من مجلد الرسائل غير المرغوب فيها أو البريد العشوائي.',
      action: 'حسنًا',
      yourRefund: 'مبلغ الاسترداد الخاص بك هو',
    },
  },
  metadata: {
    title: 'بوكاجري - السياحة الزراعية الأصيلة في الأردن',
    description:
      'استمتع بتجربة السياحة الزراعية الأصيلة في الأردن مع بوكاجري. تواصل مع المزارعين المحليين، وشارك في الأنشطة العملية، واستمتع بالإقامات الريفية. احجز مغامرتك الزراعية اليوم!',
    siteName: 'بوكاجري',
    logoAlt: 'شعار بوكاجري',
  },
  auth: {
    login: {
      title: 'تسجيل الدخول',
      welcomeTo: 'مرحبا بك في',
      emailPhone: 'البريد الإلكتروني أو رقم الهاتف',
      password: 'كلمة المرور',
      forgotPassword: 'نسيت كلمة المرور؟',
      loginButton: 'تسجيل الدخول',
      loggingIn: 'جاري تسجيل الدخول...',
      signingIn: 'جاري تسجيل الدخول...',
      loggedInSuccess: 'تم تسجيل الدخول بنجاح!',
      loginFailed: 'فشل تسجيل الدخول. يرجى المحاولة مرة أخرى.',
      noAccount: 'ليس لديك حساب؟',
      signUp: 'إنشاء حساب',
      or: 'أو',
      verificationRequired: 'يرجى التحقق من بريدك الإلكتروني للمتابعة',
      accountNotActive: 'الحساب غير نشط. يرجى التواصل مع الدعم.',
      loginRequired: 'تسجيل الدخول مطلوب',
      loginRequiredMessage: 'يرجى تسجيل الدخول للمتابعة مع حجزك.',
      loginToAddFavorites: 'يرجى تسجيل الدخول لإضافة العناصر إلى مفضلاتك.',
      cancel: 'إلغاء',
    },
    signup: {
      title: 'إنشاء حساب',
      createAccount: 'إنشاء حسابك مع',
      firstName: 'الاسم الأول',
      lastName: 'اسم العائلة',
      email: 'البريد الإلكتروني',
      phoneNumber: 'رقم الهاتف',
      password: 'كلمة المرور',
      confirmPassword: 'تأكيد كلمة المرور',
      agreeToTerms: 'أوافق على',
      privacyPolicy: 'سياسة الخصوصية',
      and: 'و',
      termsConditions: 'الشروط والأحكام',
      createButton: 'إنشاء حساب',
      creatingAccount: 'جاري إنشاء الحساب...',
      accountCreatedSuccess: 'تم إنشاء الحساب بنجاح!',
      creationFailed: 'فشل في إنشاء الحساب. يرجى المحاولة مرة أخرى.',
      alreadyHaveAccount: 'لديك حساب بالفعل؟',
      login: 'تسجيل الدخول',
    },
    forgotPassword: {
      title: 'نسيت كلمة المرور',
      titleColored: 'كلمة المرور؟',
      description:
        'أدخل عنوان البريد الإلكتروني المرتبط بحسابك، وسنرسل لك رابطاً لإعادة تعيين كلمة المرور',
      email: 'البريد الإلكتروني',
      sendButton: 'إرسال',
      sendingCode: 'جاري إرسال الرمز...',
      verificationSent: 'تم إرسال رمز التحقق إلى بريدك الإلكتروني!',
      sendFailed: 'فشل إرسال رمز التحقق. يرجى المحاولة مرة أخرى.',
    },
    resetPassword: {
      title: 'إعادة تعيين كلمة المرور',
      newPassword: 'كلمة المرور الجديدة',
      confirmPassword: 'تأكيد كلمة المرور',
      resetButton: 'إعادة تعيين كلمة المرور',
      resetting: 'جاري إعادة تعيين كلمة المرور...',
      resetSuccess: 'تم إعادة تعيين كلمة المرور بنجاح!',
      resetFailed: 'فشل إعادة تعيين كلمة المرور. يرجى المحاولة مرة أخرى.',
    },
    verify: {
      title: 'التحقق من الحساب',
      description: 'لقد أرسلنا رمز تحقق إلى بريدك الإلكتروني',
      code: 'رمز التحقق',
      verifyButton: 'تحقق',
      verifying: 'جاري التحقق...',
      resendCode: 'إعادة إرسال الرمز',
      didntReceiveCode: 'لم تستلم الرمز؟',
      verifySuccess: 'تم التحقق من الحساب بنجاح!',
      verifyFailed: 'فشل التحقق من الحساب. يرجى المحاولة مرة أخرى.',
      dataProcessingError:
        'خطأ في معالجة البيانات. يرجى المحاولة مرة أخرى لاحقاً.',
    },
    welcome: {
      title: 'مرحباً بك في بوكاجري',
      description: 'أكمل ملفك الشخصي للبدء',
    },
    profile: {
      completionRequired: 'إكمال الملف الشخصي مطلوب',
      missingFields: 'الحقول المفقودة',
      completeProfile: 'إكمال الملف الشخصي',
      profileCompletionTitle: 'أكمل ملفك الشخصي',
      profileCompletionDescription:
        'يرجى تقديم معلومات إضافية لإكمال ملفك الشخصي',
    },
    firebase: {
      signInCancelled: 'تم إلغاء تسجيل الدخول',
      popupBlocked: 'يرجى السماح للنوافذ المنبثقة لهذا الموقع',
      unauthorizedDomain: 'هذا النطاق غير مخول للمصادقة',
      signInFailed: 'فشل تسجيل الدخول باستخدام',
      unsupportedProvider: 'مزود غير مدعوم',
    },
  },
  helpCenter: {
    title: 'مركز المساعدة',
    needHelp: 'تحتاج مساعدة؟',
    sendMessage: 'أرسل لنا رسالة وسنرد عليك قريباً!',
    subject: 'الموضوع',
    messagePlaceholder: 'اكتب رسالتك هنا!',
    attachFile: 'إرفاق ملف / لقطة شاشة',
    save: 'حفظ',
    saving: 'جاري الحفظ...',
    checkFAQs: 'يمكنك أيضاً الاطلاع على',
    faqs: 'الأسئلة الشائعة',
    inReview: 'قيد المراجعة',
    reviewStatus: 'حالة المراجعة',
    websiteReply: 'رد الموقع',
    thankYouMessage: 'شكراً لرسالتك. لقد استلمنا طلبك وسنرد عليك قريباً.',
    needHelpButton: 'تحتاج مساعدة',
    contactUs: 'اتصل بنا',
    fillFields: 'يرجى ملء جميع الحقول.',
    errorMessage: 'فشل إنشاء التذكرة. يرجى المحاولة مرة أخرى.',
    fileUploadError: 'فشل رفع الملف. يرجى المحاولة مرة أخرى.',
  },
  filters: {
    showMore: 'عرض المزيد',
    showLess: 'عرض أقل',
  },
  common: {
    cancel: 'إلغاء',
    save: 'حفظ',
    saving: 'جاري الحفظ...',
    add: 'إضافة',
    adding: 'جاري الإضافة...',
    create: 'إنشاء',
    creating: 'جاري الإنشاء...',
    readMore: 'اقرأ المزيد',
    readLess: 'اقرأ أقل',
    goToHome: 'اذهب للرئيسية',
    done: 'تم',
    error: {
      generic: 'حدث خطأ. يرجى المحاولة مرة أخرى.',
    },
  },
  favorites: {
    addToCollection: 'إضافة إلى المجموعة',
    createNewCollection: 'إنشاء مجموعة جديدة',
    selectExistingCollection: 'اختيار مجموعة موجودة',
    collectionName: 'اسم المجموعة',
    enterCollectionName: 'أدخل اسم المجموعة',
    addToSelected: 'إضافة إلى المجموعة المحددة',
    items: 'عناصر',
    alreadyInFavorites: 'هذا العنصر موجود بالفعل في مفضلاتك',
    addedSuccessfully: 'تمت الإضافة إلى المجموعة بنجاح!',
    addError: 'فشل في الإضافة إلى المجموعة. يرجى المحاولة مرة أخرى.',
  },
  filter: {
    'time-of-day': 'وقت اليوم',
    'experience-duration': 'مدة التجربة',
    'age-suitability': 'المناسب للعمر',
    'experience-level': 'مستوى التجربة',
    'price-range': 'نطاق السعر',
    'average-price': 'متوسط سعر التجربة هو $',
    minimum: 'الحد الأدنى',
    maximum: 'الحد الأقصى',
    'language-preference': 'تفضيل اللغة',
    'show-more': 'عرض المزيد',
    'booking-verified': 'حجز! تم التحقق',
    'available-amenities': 'المرافق المتاحة',
    'special-offers': 'العروض الخاصة',
    'booking-options': 'خيارات الحجز',
    'accessibility-features': 'ميزات إمكانية الوصول',
    yes: 'نعم',
    no: 'لا',
    filter: 'تصفية',
    'clear-all': 'مسح الكل',
    'show-results': 'عرض النتائج',
    any: 'أي',
    'includes-experience': 'يشمل التجربة',
    bedrooms: 'غرف النوم',
    beds: 'الأسرة',
    bathrooms: 'الحمامات',
    'package-duration': 'مدة الباقة',
  },
  booking: {
    financialReceipt: {
      error: 'يرجى رفع إيصال المالي',
      success: 'تم إنشاء الحجز بنجاح!',
    },
    paymentMethod: {
      error: 'يرجى اختيار طريقة الدفع',
    },
    guests: {
      selectGuests: 'يرجى اختيار الضيوف',
      exceedsLimit: 'عدد الضيوف المحدد يتجاوز الحد المسموح',
    },
    success: {
      title: 'حالة الحجز',
      message: 'حجزك قيد الانتظار حالياً.',
      description:
        'سنقوم بمراجعته قريباً، وبمجرد التأكيد، ستكون جاهزاً لإقامتك الزراعية.',
      emailNote:
        'إذا لم تستلم البريد الإلكتروني، يرجى التحقق من مجلد الرسائل غير المرغوب فيها أو البريد العشوائي.',
      viewBooking: 'حسناً',
    },
    availableStaySlots: 'فترات الإقامة المتاحة',
    availableTimeSlots: 'الأوقات المتاحة',
    startFrom: 'يبدأ من',
    perPerson: '/شخص',
    contactUsToRefund: 'اتصل بنا للاسترداد',
    reservationDetails: 'تفاصيل الحجز',
    paymentInformation: 'تفاصيل الدفع',
    datesAndTime: 'التواريخ والوقت',
    location: 'الموقع',
    amenities: 'المرافق',
    experience: 'التجربة',
    hosts: 'المضيفين',
    whoIsComing: 'من يأتي',
    show: 'عرض',
    paymentDetails: 'تفاصيل الدفع',
    totalCost: 'التكلفة الإجمالية',
    paymentMethods: 'طريقة الدفع',
    choose: 'اختيار',
    adult: 'البالغ',
    children: 'الأطفال',
    infant: 'الرضع',
    selectGuests: 'اختر الضيوف',
    AttachTheFinancialReceipt: 'إرفاق إيصال المالي',
    Submit: 'إرسال',
  },
  search: {
    'check-in': 'تسجيل الوصول',
    'check-out': 'تسجيل المغادرة',
    'add-date': 'إضافة تاريخ',
    'add-guests': 'إضافة ضيوف',
    search_for_country: 'ابحث عن دولة...',
    search: 'بحث',
    when: 'متى',
    who: 'من',
    where: 'الوجهة',
    destination: 'الوجهة',
  },
  form: {
    date: 'التاريخ',
    guests: 'الضيوف',
    noCountriesAvailable: 'لا توجد دول متاحة',
    noCountriesFound: 'لا توجد دول مطابقة',
    searchForCountry: 'ابحث عن دولة...',
  },
  validation: {
    pleaseSelectCountry: 'يرجى اختيار الدولة',
    pleaseSelectCity: 'يرجى اختيار المدينة',
  },
  regions: {
    menaRegion: 'منطقة الشرق الأوسط وشمال أفريقيا',
    imFlexible: 'مرن',
    jordan: 'الأردن',
    europe: 'أوروبا',
    africa: 'أفريقيا',
    city: 'مدينة',
  },
  map: {
    showMapView: 'عرض الخريطة',
  },
  datePicker: {
    add_date: 'إضافة تاريخ',
    selectDate: 'اختر التاريخ',
    months: {
      january: 'يناير',
      february: 'فبراير',
      march: 'مارس',
      april: 'أبريل',
      may: 'مايو',
      june: 'يونيو',
      july: 'يوليو',
      august: 'أغسطس',
      september: 'سبتمبر',
      october: 'أكتوبر',
      november: 'نوفمبر',
      december: 'ديسمبر',
    },
    days: {
      saturday: 'السبت',
      sunday: 'الأحد',
      monday: 'الاثنين',
      tuesday: 'الثلاثاء',
      wednesday: 'الأربعاء',
      thursday: 'الخميس',
      friday: 'الجمعة',
    },
    dayAbbreviations: {
      sa: 'سبت',
      su: 'أحد',
      mo: 'اثنين',
      tu: 'ثلاثاء',
      we: 'أربعاء',
      th: 'خميس',
      fr: 'جمعة',
    },
  },
  loadMore: 'عرض المزيد',
  showAllPhotos: 'عرض جميع الصور',
  searchResults: {
    noResults: 'لا يوجد نتائج',
    searching: 'جاري البحث...',
    noResultsFound: 'لا يوجد نتائج',
    searchingFor: 'جاري البحث عن',
    resultsFor: 'نتائج ل',
    clearSearch: 'مسح البحث',
    searchAgain: 'ابحث مرة أخرى',
    searchAgainPlaceholder: 'ابحث عن مدينة أو دولة...',
    startTypingToSearch: 'ابدأ الكتابة للبحث...',
  },
  cancellationPolicy: {
    cancelUpTo: 'إلغاء حتى',
    hoursBeforeTheStartTimeForAfullRefund:
      'ساعات قبل بدء الوقت للاسترداد الكامل',
  },
  payment: {
    success: {
      title: 'تم الدفع بنجاح!',
      subtitle: 'نجحت المعاملة',
      description:
        'تم حجز تجربة السياحة الزراعية بنجاح. لقد أرسلنا لك بريداً إلكترونياً يحتوي على جميع التفاصيل. استعد لمغامرة زراعية رائعة!',
    },
    declined: {
      title: 'تم رفض الدفع',
      subtitle: 'فشل في المعاملة',
      description:
        'لم نتمكن من معالجة دفعتك. يرجى التحقق من تفاصيل الدفع والمحاولة مرة أخرى. إذا استمرت المشكلة، اتصل بالبنك أو جرب طريقة دفع أخرى.',
      tryAgain: 'حاول مرة أخرى',
    },
    cancelled: {
      title: 'تم إلغاء الدفع',
      subtitle: 'تم إلغاء المعاملة',
      description:
        'لقد ألغيت عملية الدفع. لم يتم تأكيد حجزك. يمكنك متابعة تصفح التجارب أو إعادة المحاولة عندما تكون مستعداً.',
      continueBooking: 'متابعة الحجز',
    },
    backToHome: 'العودة للرئيسية',
  },
  location: {
    location: 'الموقع',
    exactLocation: 'سيكون الموقع الدقيق متاحًا بمجرد الموافقة على التجربة.',
    itinerary: 'الجدول الزمني',
    typicalItinerary: 'هذا هو الجدول الزمني النموذجي لهذا الباقة',
    stopAt: 'توقف عند',
    hour: 'ساعة',
    hours: 'ساعات',
  },
  overview: {
    overview: 'الملخص',
  },
  amenities: {
    availableAmenities: 'المرافق المتاحة',
    seeAll: 'عرض الكل',
    allAmenities: 'جميع المرافق',
  },
  whatToExpect: {
    whatToExpect: 'ما الذي ينتظرك',
    readLess: 'اقرأ أقل',
    readMore: 'اقرأ المزيد',
  },
  specialInstructions: {
    specialInstructions: 'التعليمات الخاصة',
    cancellationPolicy: 'سياسة الإلغاء',
  },
  similarExperiences: {
    similarExperiences: 'التجارب المماثلة',
    ifYouEnjoyedThis: 'إذا أعجبك هذا, قد تعجبك أيضاً',
  },
  stayDetails: {
    stayDetails: 'تفاصيل الإقامة',
  },
  nearbySurroundings: {
    nearbySurroundings: 'المحيط والمناطق المجاورة',
  },
  houseRules: {
    houseRules: 'قواعد المنزل',
  },
  completeYourBooking: {
    completeYourBookingAndPay: 'اكمل حجزك وقم بالدفع',
    thingsToKnow: 'الأمور التي يجب أن تعرفها',
    stayNearby: 'الإقامة القريبة',
    stayHouseRules: 'قواعد الإقامة في المنزل',
    cancellationPolicy: 'سياسة الإلغاء',
    guide: 'الدليل',
    transportation: 'النقل',
    airport: 'المطار',
    mandatory: 'مطلوب',
    optional: 'اختياري',
    priceBreakdown: 'تفاصيل السعر',
    total: 'الإجمالي',
    currency: 'العملة',
    totalAmount: 'المبلغ الإجمالي',
    discount: 'الخصم',
    tax: 'الضريبة',
    subtotal: 'المبلغ الإجمالي',
    choosePaymentMethods: 'اختر طرق الدفع',
    financialReceiptRequired: 'يجب إرفاق إيصال المالي لهذه الطريقة الدفع',
  },
};

export default ar;
