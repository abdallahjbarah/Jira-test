...
const SUGGESTIONS_API_URL = 'https://api.staging.bookagri.com/sites/suggestions?search=';
...
const WherePopup: React.FC<WherePopupProps> = ({ isOpen, onClose, currentCollectionStatus, onNext, onClear, onFilterSelect, filtersValue }) => {
    ...
    const { data: initialSuggestions, isLoading: isLoadingInitialSuggestions, error: initialSuggestionsError } = useQuery({
    ...
    const handleNext = () => {
        ...
        if (selectedLocation) {
            if (selectedLocation.type === 'site') {
                router.push(`/${searchParams.get('lang') || 'en'}/details/${selectedLocation.id}`);
                onClose();
            } else {
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleClose} width="400px" canClose>
            <div className="flex flex-col" style={{ minHeight: '350px', maxHeight: '420px', overflow: 'hidden' }}>
                <div className="px-4 pt-4 flex-1 overflow-hidden">
                                        ...
                                        <>
                                            {filter.icon && (
                                                <Image
                                                    ...
                                                    width={26}
                                                    height={26}
                                                    className={`mb-0.5 transition-all ${isSelected
                    ...
                    <div className="mt-6 mb-4">
                        ...
                        {showSuggestions && hasTyped && searchResults && searchResults.results?.length > 0 && (
                                ...
                                <div className="max-h-60 overflow-y-auto">
                                    {searchResults.results.map((result: any, idx: number) => (
                                        <div
                                            ...
                                            style={{ ...(idx !== 0 ? {} : {}), transition: 'background 0.2s, color 0.2s', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}
                                            onMouseEnter={e => {
                                                e.currentTarget.style.background = '#47C409';
                                                e.currentTarget.style.color = '#fff';
                                                Array.from(e.currentTarget.querySelectorAll('span, svg, img')).forEach(el => {
                                                    if (el.tagName === 'SPAN' && !el.classList.contains('type-badge')) (el as HTMLElement).style.color = '#fff';
                                                    if (el.tagName === 'IMG') (el as HTMLImageElement).style.filter = 'brightness(0) invert(1)';
                                                });
                                            }}
                                            onMouseLeave={e => {
                                                ...
                                                Array.from(e.currentTarget.querySelectorAll('span, svg, img')).forEach(el => {
                                                    if (el.tagName === 'SPAN' && !el.classList.contains('type-badge')) (el as HTMLElement).style.color = '';
                                                    if (el.tagName === 'IMG') (el as HTMLImageElement).style.filter = '';
                                                });
                                            }}
                                            onClick={() => handleSearchResultSelect(result)}
                                        >
                                            <Image src="/SVGs/shared/map-icon.svg" alt="Location" width={20} height={20} className="opacity-60 flex-shrink-0" />
                        ...
                        {showSuggestions && !hasTyped && initialSuggestions && (initialSuggestions as any)?.results?.length > 0 && (
                            <div className="absolute z-10 left-0 w-full bg-white border border-gray-300 rounded-lg mt-2 shadow-lg">
                                <div className="max-h-60 overflow-y-auto">
                                    {(initialSuggestions as any).results.map((suggestion: any, idx: number) => (
                                        <div
                                            key={suggestion._id || suggestion.cityId || suggestion.country}
                                            className={`flex items-center gap-3 px-4 py-2 cursor-pointer transition-colors w-full shadow-md rounded-lg bg-white ${idx !== 0 ? 'border-t border-gray-200' : ''}`}
                                            style={{ ...(idx !== 0 ? {} : {}), transition: 'background 0.2s, color 0.2s', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}
                                            onMouseEnter={e => {
                                                e.currentTarget.style.background = '#47C409';
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-2 mt-0.5">
                                                    {(suggestion.city || suggestion.country) && (
                                                        <span className="truncate block w-full overflow-hidden whitespace-nowrap text-xs text-gray-500">
                                                            {suggestion.city && suggestion.country ? `${suggestion.city}, ${suggestion.country}` : (suggestion.city || suggestion.country)}
                                                        </span>
                                                    )}
                ...
                <div className="px-4 pb-6 pt-4 border-t border-gray-100 mt-2">
                    <div className="flex justify-center">
                        <button
                            onClick={handleNext}
                            disabled={!searchText.trim() && !selectedLocation && selected === 'all'}
            ...
            <style jsx global>{`
              .type-badge { color: #FE3B0A !important; }
              .type-badge:hover { color: #FE3B0A !important; }
            `}</style>
        </Modal>
    );
};
