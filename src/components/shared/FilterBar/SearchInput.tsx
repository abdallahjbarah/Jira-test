...
const SearchInput: React.FC<SearchInputProps> = ({ value, onChange }) => {
  ...
  const [selectedOption, setSelectedOption] = React.useState<{ label: string; value: string; data?: any } | null>(
    value ? { label: value, value } : null
  );

  // Create a debounced function to update the debouncedInput
  ...
  // Update debouncedInput when inputValue changes
  React.useEffect(() => {
    debouncedSetInput(inputValue);
  }, [inputValue, debouncedSetInput]);

  const { data: searchData, isLoading } = useFetchSearchDestination(
    debouncedInput
  );
  ...
  const options = React.useMemo(() => {
    ...
    return searchData.results.map((item: any) => {
      ...
      return {
        label,
        value: label,
        data: item // Keep original data for custom option rendering
      };
    });
  }, [searchData?.results]);
  ...
  const handleInputChange = (newValue: string) => {
    ...
    // Only open menu if there's input and we have some results or are loading
  ...
  const handleChange = (option: any) => {
    if (option) {
      // If the selected option is a site, redirect to details page
      ...
      // For city and country, set the value normally
      setInputValue(option.label);
      setSelectedOption(option);
      
      // Set the appropriate filter value based on search type
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col items-center gap-2">
          <div className="flex items-center gap-2">
            <Image src={ExperienceIcon} alt="Experience" width={24} height={24} className="min-w-6 min-h-6" />
  ...
  const noOptionsMessage = ({ inputValue }: { inputValue: string }) => {
    ...
    if (inputValue.length === 0) return "Start typing to search...";
